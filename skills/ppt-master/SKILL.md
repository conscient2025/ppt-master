...
|---|---|---|
| `ai` | `references/image-generator.md` | Codex sessions: host-native image generation; non-Codex/API sessions: `python3 ${SKILL_DIR}/scripts/image_gen.py --manifest <project_path>/images/image_prompts.json` |
...
> ⚠️ **In-pipeline ai path MUST use the manifest contract** — even when only 1 ai row exists. Write `images/image_prompts.json` first, generate via the selected path in `references/image-generator.md`, then ensure `image_prompts.md` is rendered from the manifest. In Codex sessions, prefer the host-native image tool and update manifest statuses manually; do not call `image_gen.py` unless the user explicitly requests API/backend generation. The positional form (`image_gen.py "prompt" ...`) is reserved for **out-of-pipeline one-off testing / single-image fixups** — it skips manifest + sidecar, leaving no audit trail.
